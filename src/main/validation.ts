...
// no API-key variant exists for these in athena-agent's registry.
...
// athena-agent pick at runtime; the others are local self-hosted
...
export function validateChatReadiness(profile?: string): ChatReadiness {
  try {
    ...
    // Provider="auto" lets athena-agent pick a model at runtime based
