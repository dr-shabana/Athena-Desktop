 ...
 * ~/.cortex/desktop/sessions.json; named profiles use
 * ~/.cortex/profiles/<name>/desktop/sessions.json (issue #311).
 */
...
// Sync from athena DB to local cache — only fetches new/updated sessions
export function syncSessionCache(): CachedSession[] {
  ...
  try {
    ...
    if (staleIds.length > 0) {
      ...
      // portability across the better-sqlite3 versions athena ships.
      const CHUNK = 500;
