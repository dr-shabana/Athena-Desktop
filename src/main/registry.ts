 ...
 *   https://github.com/dr-shabana/athena-registry
 *
 ...
 */
const REGISTRY_REPO = "dr-shabana/athena-registry";
...
interface EntryManifest {
  ...
  tools?: string[];
  license?: string;
  compatibility?: { athena?: string; desktop?: string } | null;
}
...
function buildSpec(
  ...
  if (compat?.athena) {
    rows.push({ label: "Requires Athena", value: compat.athena, mono: true });
  }
 ...
 * exact shape the engine's config loader expects (see athena-agent
 ...
 *                via `athena skills install <source>`)
