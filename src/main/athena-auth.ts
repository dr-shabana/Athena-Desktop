 ...
 * (`athena auth add <provider> --type oauth`) rather than a static API
 * key. Mirrors athena-agent's `_OAUTH_CAPABLE_PROVIDERS` set.
 *
...
/**
 * Run `athena auth add <provider> --type oauth`, streaming the CLI's
 ...
 */
export function runAthenaAuthLogin(
  provider: string,
  ...
  return new Promise((resolve) => {
    ...
    try {
      proc = spawn(CORTEX_PYTHON, athenaCliArgs(subArgs), {
        cwd: CORTEX_REPO,
        env: {
          ...process.env,
 ...
 */
export function cancelAthenaAuthLogin(): boolean {
