...
function Sessions({
  ...
  const refreshSessions = useCallback(async (): Promise<void> => {
    const synced = await window.athenaAPI.syncSessionCache();
  ...
  const loadSessions = useCallback(async (): Promise<void> => {
    setLoading(true);
    try {
      const cached = await window.athenaAPI.listCachedSessions(50);
      if (cached.length > 0) {
        setSessions(cached);
      }
  ...
  const confirmRename = useCallback(
    async (sessionId: string, newTitle: string): Promise<void> => {
      ...
      try {
        await window.athenaAPI.updateSessionTitle(sessionId, trimmed);
      } catch (err) {
  ...
  const confirmDelete = useCallback(
    async (sessionId: string): Promise<void> => {
      ...
      try {
        await window.athenaAPI.deleteSession(sessionId);
      } catch (err) {
  ...
  const confirmBulkDelete = useCallback(
    async (sessionIds: string[]): Promise<void> => {
      ...
      try {
        await window.athenaAPI.deleteSessions(ids);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      try {
        const results = await window.athenaAPI.searchSessions(query);
