...
vi.mock("../../components/common/AthenaLogo", () => ({
  default: (): React.JSX.Element => <div data-testid="athena-logo" />,
}));
...
function profile(name: string, isDefault = false): ProfileInfo {
  return {
    name,
    path: isDefault ? "C:/athena" : `C:/athena/profiles/${name}`,
    isDefault,
    isActive: isDefault,
    model: "",
    provider: "auto",
...
describe("Agents profile creation", () => {
  it("refreshes profiles after a failed create so ambiguous successes appear", async () => {
    ...
    api.createProfile.mockResolvedValue({
      success: false,
      error:
        "Error: Profile 'test2' already exists at C:/athena/profiles/test2",
    });
