...
function Schedules({ profile }: SchedulesProps): React.JSX.Element {
  ...
  const loadJobs = useCallback(async (): Promise<void> => {
    try {
      const list = await window.athenaAPI.listCronJobs(true, profile);
      setJobs(list);
    } catch {
  ...
  async function handleCreate(): Promise<void> {
    ...
    setError("");
    try {
      const result = await window.athenaAPI.createCronJob(
        buildSchedule(),
  ...
  async function handleRemove(jobId: string): Promise<void> {
    ...
    setError("");
    try {
      const result = await window.athenaAPI.removeCronJob(jobId, profile);
  ...
  async function handleToggle(job: CronJob): Promise<void> {
    ...
    try {
      const result =
        job.state === "paused"
          ? await window.athenaAPI.resumeCronJob(job.id, profile)
          : await window.athenaAPI.pauseCronJob(job.id, profile);
      if (result.success) {
        await loadJobs();
      } else {
  ...
  async function handleTrigger(jobId: string): Promise<void> {
    ...
    setError("");
    try {
      const result = await window.athenaAPI.triggerCronJob(jobId, profile);
      if (result.success) {
        await loadJobs();
      } else {
