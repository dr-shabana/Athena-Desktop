...
function Soul({ profile }: SoulProps): React.JSX.Element {
  ...
  const loadSoul = useCallback(async (): Promise<void> => {
    ...
    setLoading(true);
    const text = await window.athenaAPI.readSoul(profile);
    setContent(text);
    setLoading(false);
    setTimeout(() => {
      loaded.current = true;
  ...
  const saveSoul = useCallback(
    async (text: string) => {
      ...
      await window.athenaAPI.writeSoul(text, profile);
      setSaved(true);
  ...
  async function handleReset(): Promise<void> {
    const newContent = await window.athenaAPI.resetSoul(profile);
    loaded.current = false;
