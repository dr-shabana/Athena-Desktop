...
<br/>
<p align="center">
  <a href="https://athena-agent.dr-shabana.com/docs/"><img src="https://img.shields.io/badge/Docs-athena--agent.dr-shabana.com-FFD700?style=for-the-badge" alt="文档"></a>
  ...
  <a href="https://athenaagents.cc/"><img src="https://img.shields.io/badge/Download-Releases-FF6600?style=for-the-badge" alt="下载"></a>
<a href="https://github.com/dr-shabana/Athena-Desktop/stargazers">
  ...
  <img src="https://img.shields.io/github/downloads/dr-shabana/Athena-Desktop/total?style=for-the-badge&color=00B496&label=Total%20Downloads" alt="下载量">
</a>
</p>

> **本项目处于活跃开发阶段。** 功能可能会发生变化，某些功能也可能会失效。如果您遇到问题或有好的想法，请 [提交 Issue](https://github.com/dr-shabana/Athena-Desktop/issues)。欢迎贡献代码！
...
Athena Q 是一款原生的桌面应用程序，用于安装、配置并与 [Athena Agent](https://github.com/dr-shabana/athena-agent) 进行聊天——这是一款具备工具调用、多平台消息传递和闭环学习能力的自我进化的 AI 助手。

无需手动管理命令行界面 (CLI)，该应用可在一个统一界面中引导您完成安装、提供商设置以及日常使用。它使用官方的 Athena 安装脚本，将 Athena 存储在 `~/.athena` 目录下，并为您提供涵盖聊天、会话、配置、记忆、技能、工具、计划任务、消息网关等功能的图形界面。
...
> ```
>
> 详情请见 [#109](https://github.com/dr-shabana/Athena-Desktop/issues/109)。
...
- **向导式初次安装**：带有进度跟踪和依赖解析的 Athena Agent 引导安装。
- **本地或远程后端**：可在本地 `127.0.0.1:8642` 运行 Athena，或通过 URL 和 API 密钥将桌面应用连接到远程的 Athena API 服务器。
...
- **配置切换 (Profile)**：创建、删除并切换不同的 Athena 环境，配置完全隔离。
...
- **Athena 办公室 (Claw3d)**：具有开发服务器和适配器管理功能的可视化 3D 界面。
...
1. 询问您是希望在**本地**运行 Athena，还是连接到**远程**的 Athena API 服务器。
2. **本地模式：** 检查 `~/.athena` 目录下是否已安装 Athena；如果未安装，则运行官方 Athena 安装脚本并解决依赖关系 (Git, uv, Python 3.11+)。
3. **远程模式：** 提示输入远程 API URL 和 API 密钥，验证连接，并跳过本地安装。
4. 提示输入 API 提供商或本地模型端点。
5. 将提供商配置和 API 密钥保存至 Athena 配置文件。
6. 设置完成后启动主工作区。
...
| **代理 (Agents)**    | 创建、删除和在不同的 Athena 配置 (Profile) 之间切换                                    |
...
- Node.js 和 npm
- 能够运行 Athena 安装程序的类 Unix Shell 环境
- 首次运行安装 Athena 时需要网络连接
...
当应用首次打开时，它会自动检测是否存在已安装的 Athena 实例，或者提供帮您进行自动安装的选项。
...
Athena 的相关文件统一管理于以下目录：

- `~/.athena`
- `~/.cortex/.env`
- `~/.cortex/config.yaml`
- `~/.cortex/athena-agent`
- `~/.cortex/profiles/` — 命名配置文件目录
- `~/.cortex/state.db` — 会话历史数据库
- `~/.cortex/cron/jobs.json` — 计划任务
...
- 此桌面应用依赖上游的 Athena Agent 项目来处理代理行为和工具执行。
- 内置安装程序会通过带有 `--skip-setup` 参数的方式运行官方 Athena 安装脚本，然后在 GUI 界面中完成提供商相关的配置。
- 本地模型提供商不需要 API 密钥，但您必须确保兼容的服务器已经在运行中。
- 在网络受限的环境下，支持配置备用的 npm 镜像源路由。

## 参与贡献

欢迎大家参与贡献！查看 [参与贡献指南 (Contributing Guide)](CONTRIBUTING.md) 以开始。如果您不知从何入手，可以看一看 [开启的 Issues](https://github.com/dr-shabana/Athena-Desktop/issues)。发现了 Bug 或是对功能有新的需求？ [提交一个 Issue](https://github.com/dr-shabana/Athena-Desktop/issues/new)。

## 相关项目

如果想了解核心代理功能、详细文档及命令行 (CLI) 的工作流程，请查阅主仓库 Athena Agent：
